...
router.post('/forgotPassword', (req,res)=>{
    ...
    connection.query(query,[user.email],(err,results)=>{
        if(!err){
            if(results.length <=0){
                    return res.status(200).json({message :"Password sent successully to your Email."})
            }else{
                var mailOptions ={
...
// checkRole.checkRole -->for admin user it will give data otherwise 
...
//For updating the status of the User.
    ...
    connection.query(query,[user.status,user.id],(err,results)=>{
        if(!err){
            if(results.affectedRows ==0){
                return res.status(404).json({message:"User id does not exit"})
            }else{
